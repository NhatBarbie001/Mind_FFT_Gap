...
from .loralib import layers_fft as lora_fft
...
class LoRAResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, r=4, only_kv=False,mlp=False, n_frq=3000, n_tasks=10, device=None):
        super().__init__()

        self.attn = lora_fft.MultiheadAttention_FFT(d_model, n_head, r=r, only_kv=only_kv, mlp=mlp, n_frq=n_frq, n_tasks=n_tasks, device=device) # LoRA rank set as 4
        self.ln_1 = LayerNorm(d_model)
        if only_kv:
            self.mlp = nn.Sequential(OrderedDict([
                ("c_fc", nn.Linear(d_model, d_model * 4)),
...
class LoRATransformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, r = 4, only_kv=False, mlp=False, n_frq=3000, n_tasks=10, device=None):
        super().__init__()
        self.width = width
        self.layers = layers
...
class LoRAVisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, r: int, only_kv=False, mlp=False, n_frq=3000, n_tasks=10, device=None):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        if only_kv:
        ...
        self.transformer = LoRATransformer(width, layers, heads, only_kv=only_kv, r=r, mlp=mlp, n_frq=n_frq, n_tasks=n_tasks, device=device)
        # self.transformer = Transformer(width, layers, heads)
...
def build_LoRA_FFT_model(state_dict: dict, r: int, lora_mode: str, n_frq: int = 3000, n_tasks: int = 10, device=None):
    vit = "visual.proj" in state_dict
    ...
    model = LoRACLIP(
        ...
        image_resolution, vision_layers, vision_width, vision_patch_size,
        context_length, vocab_size, transformer_width, transformer_heads, transformer_layers, 
        r, lora_mode, n_frq=n_frq, n_tasks=n_tasks, device=device
    )
